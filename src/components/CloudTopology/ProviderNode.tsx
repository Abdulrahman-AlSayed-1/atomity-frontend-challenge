...
/**
 * ProviderNode — octagonal cloud provider node with pods inside.
 * Uses @container queries for pod grid layout responsiveness.
 */
export function ProviderNode({ provider, animate, index }: ProviderNodeProps) {
 ...
 const isBottom = provider.position.includes("bottom");

 const delayBase = 0.2 + index * 0.12;

 // Resolve color from CSS var name
 ...
 animate={animate ? {
 boxShadow: [
 `inset 0 0 10px ${resolvedColor}10`,
 `inset 0 0 25px ${resolvedColor}20`,
 `inset 0 0 10px ${resolvedColor}10`,
 ],
 } : {}}
 ...
 {/* Pod grid — uses @container for responsive layout */}
 <div
 className="relative z-10 flex flex-wrap items-center justify-center gap-2 p-4"
 style={{ maxWidth: 90 }}
 >
