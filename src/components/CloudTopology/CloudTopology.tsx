...
export function CloudTopology() {
  ...
  return (
    <section
      ref={ref}
      className="relative w-full overflow-x-clip flex flex-col items-center justify-center"
      style={{
        paddingTop: "64px",
        paddingBottom: "64px",
      }}
    ...
    >
      ...
      <div className="relative z-10 max-w-6xl mx-auto w-full">
        ...
        >
          ...
          <h2 id="topology-heading" className="heading-section" style={{ color: "var(--color-text-primary)" }}>
            One view across <span style={{ color: "var(--color-accent-green)" }}>every cloud</span>
          </h2>

          <p className="max-w-xl text-base" style={{ color: "var(--color-text-secondary)", fontSize: "var(--text-lg)" }}>
            Atomity unifies your infrastructure across AWS, Azure, GCP, and on-premise — revealing cost inefficiencies and savings opportunities in real time.
          </p>

          <div className="flex flex-wrap justify-center gap-2 mt-2">
            <Badge variant="success" dot>Live data</Badge>
            <Badge variant="default">4 providers</Badge>
            <Badge variant="savings">${data?.totalSavings?.toLocaleString() ?? "…"}/mo savings</Badge>
          </div>
        </motion.div>

        {/* Topology canvas */}
        <div className="topology-grid relative py-4" style={{ minHeight: 480 }}>
        ...
        >
          {[
            ...
            { label: "Resources Monitored", value: "6" },
            {
              ...
              value: data?.totalSavings ? `$${data.totalSavings.toLocaleString()}` : isLoading ? "…" : "—",
              accent: true,
            },
            ...
            <div
              ...
              style={{
                padding: "16px 16px",
                background: accent ? "var(--color-accent-savings-bg)" : "var(--color-bg-surface)",
                borderColor: accent ? "var(--color-border-strong)" : "var(--color-border-surface)",
              }}
            >
              <span className="text-2xl font-bold font-mono" style={{ color: accent ? "var(--color-accent-savings)" : "var(--color-text-primary)" }}>
                {value}
              </span>
